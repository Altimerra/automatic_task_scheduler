...
scheduler = {
  ...
  scheduleForTimeblock: function (timeblock) {
    for (i = 0; i < this.taskarray.length; i++) {
      ...
      if (
        timeblock.availableTime.timeAmount >= task.time.timeAmount &&
        task.scheduled == false
      ) {
        task.scheduled = true;
        timeblock.availableTime = timeDifference(
          timeblock.availableTime,
          task.time
        );
...
tasklist = {
  ...
  createElement: function (element) {
    ...
    badge.innerHTML =
      element.time.hours.toString() +
      "h " +
      element.time.minutes.toString() +
      "m";
  ...
  createTask: function () {
    if (this.taskname.value == "") {
      ...
      let newTask = new Task(
        this.taskname.value,
        new Time(this.taskhours.value, this.taskminutes.value)
      );
      this.taskname.value = "";
...
timeblockform = {
  ...
  createTimeblock: function () {
    ...
    if (this.timeblockend.value == "") {
      ...
      let newTimeblock = new Timeblock(
...
$(document).ready(function () {
  ...
  $("#timeblockstart").popover({
    animation: true,
    trigger: "manual",
    content: "This field is empty",
  });
  $("#timeblockend").popover({
    animation: true,
    trigger: "manual",
    content: "This field is empty",
  });
